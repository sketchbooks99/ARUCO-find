...
void drawMarker(float size, const ofColor & color, int id=NULL) {
    ofDrawAxis(size);
    ofPushMatrix();
...
//--------------------------------------------------------------
void ofApp::setup(){
    ofxPublishOsc(HOST, PORT, "/aruco/ids", ids);
...
void ofApp::update(){
    ...
    for(int i=0; i<aruco.getNumMarkers(); i++) {
        ids.push_back(aruco.getMarkers()[i].id);
    }
}
...
void ofApp::draw(){
    ...
    if(showMarkers) {
        for(int i = 0; i<aruco.getNumMarkers(); i++) {
            aruco.begin(i);
            drawMarker(0.15, ofColor::white, aruco.getMarkers()[i].id);
            aruco.end();
        }
    }
